...
// 解析和绘制K线图的主函数
function parseAndDrawKLineChart(klineDataObj) {
    // 获取容器
    ...
    // 绘制K线图 - 调整参数调用
    drawKLineChart(parsedData.dates, parsedData.klineData);
    ...
    return fetch(proxyUrl)
        ...
        .then(function(data) {
            if (data && data.data) {
                ...
                // 解析和绘制图表
                parseAndDrawKLineChart(data.data);
            } else {
        ...
        .catch(function(error) {
            ...
            // 在API失败时，使用模拟数据绘制图表
            useMockDataForAllCharts(stockName, stockCode);
        });
}

// API失败时使用模拟数据绘制所有图表
function useMockDataForAllCharts(stockName, stockCode) {
    console.log('使用模拟数据绘制图表');
    
    // 更新弹窗标题
    document.getElementById('popupStockName').textContent = `${stockName}(${stockCode}) - 模拟数据`;
    
    // 创建模拟K线数据
    var mockData = createMockKLineData(120);
    
    // 解析模拟数据
    var parsedData = {
        dates: mockData.dates,
        klineData: mockData.klineData,
        volumeData: mockData.volumeData,
        turnoverData: mockData.turnoverData
    };
...
// 创建模拟的K线数据
function createMockKLineData(days) {
    var dates = [];
    var klineData = [];
    var volumeData = [];
    var turnoverData = [];
    ...
    for (var i = days - 1; i >= 0; i--) {
        ...
        basePrice += change;
        basePrice = Math.max(50, basePrice); // 确保价格不会过低
        
        // 生成当日K线数据
        var open = basePrice;
        var close = basePrice + (Math.random() - 0.5) * 2;
        ...
        // 生成成交量数据
        var volume = 10000000 + Math.random() * 90000000; // 100万到1亿之间
        volumeData.push(volume);
        
        // 生成成交额数据
        var turnover = volume * close;
        turnoverData.push(turnover);
    }
    
    return { dates, klineData, volumeData, turnoverData };
}
...
// 绘制K线图
function drawKLineChart(dates, klineData) {
    // 初始化K线图实例
...
function setupChartLinkage() {
    if (kLineChart && volumeChart) {
        // 监听成交量图的数据缩放事件，同步到K线图（改为从成交量图监听）
...
function showKLinePopup(event, stockCode, stockName) {
    ...
    if (x + popup.offsetWidth > window.innerWidth) {
        x = event.pageX - popup.offsetWidth - 10;
    }
    if (y + popup.offsetHeight > window.innerHeight) {
        y = event.pageY - popup.offsetHeight - 10;
    }
